  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-[#2bffa0] selection:text-black font-sans overflow-x-hidden relative">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none opacity-40">
        <FlickeringGrid className="w-full h-full" squareSize={4} gridGap={6} color="#2bffa0" maxOpacity={0.25} flickerChance={0.15} />
      </div>

      {/* Header */}
      <header className="fixed top-0 w-full z-40 bg-black/80 backdrop-blur-md border-b border-white/10">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors group">
        ...
        <div className="max-w-3xl mx-auto space-y-24">
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            ...
            <div className="space-y-6 text-lg text-zinc-400 leading-relaxed font-light">
              <p>O mercado digital foi corrompido por promessas vazias e métricas de vaidade. Agências tradicionais comemoram curtidas, impressões e um volume inflado de leads desqualificados, enquanto os boletos da sua operação continuam chegando.</p>
              <p>Eles operam em modelos focados em volume sem margem. Entregam relatórios bonitos que não refletem no caixa da empresa. A verdade nua e crua é que <strong className="bg-[#2bffa0] text-black px-1.5 py-0.5 font-bold rounded-sm">curtidas não pagam boletos</strong> e leads frios apenas sobrecarregam seu time comercial. Se você quer sair desse ciclo destrutivo, <a href="https://form.respondi.app/lDg0Bifo" target="_blank" rel="noopener noreferrer" className="bg-[#2bffa0] text-black px-1.5 py-0.5 font-bold rounded-sm hover:opacity-90 transition-opacity">aplique para um diagnóstico estratégico</a> agora mesmo.</p>
            </div>
          </div>
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
            ...
            <div className="space-y-6 text-lg text-zinc-400 leading-relaxed font-light">
              <p>Vender produtos ou serviços exige uma abordagem completamente diferente. Não se trata de pescar de rede, mas de usar um rifle de precisão. Exige engenharia, inteligência de dados e um entendimento profundo da jornada de compra do seu cliente ideal.</p>
              <p>Nossa visão é clara: foco obsessivo em ROI verdadeiro, lucro líquido e previsibilidade de escala. Cada campanha, cada automação e cada linha de copy é projetada com um único objetivo: <strong className="bg-[#2bffa0] text-black px-1.5 py-0.5 font-bold rounded-sm">gerar receita sustentável</strong>. <a href="https://form.respondi.app/lDg0Bifo" target="_blank" rel="noopener noreferrer" className="bg-[#2bffa0] text-black px-1.5 py-0.5 font-bold rounded-sm hover:opacity-90 transition-opacity">Instale essa máquina de vendas</a> na sua empresa.</p>
            </div>
          </div>
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
            ...
            <div className="space-y-6 text-lg text-zinc-400 leading-relaxed font-light">
              <p>Nosso trabalho não termina quando o lead cai no CRM. Ele termina no faturamento. Nós assumimos a responsabilidade pela qualidade do ecossistema que construímos.</p>
              <p>É por isso que estruturamos a nossa operação de forma clara, pragmática e focada em resultados reais: <strong>1 mês dedicado a arrumar a casa</strong> com implantação completa de Inteligência Local e Google Meu Negócio, seguido por <strong>3 meses de tração agressiva e tráfego focado em ROI absoluto</strong>. Não acreditamos em milagres; acreditamos em engenharia de vendas e otimização contínua. <a href="https://form.respondi.app/lDg0Bifo" target="_blank" rel="noopener noreferrer" className="bg-[#2bffa0] text-black px-1.5 py-0.5 font-bold rounded-sm hover:opacity-90 transition-opacity">Inicie a transformação do seu ecossistema.</a></p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 px-6 border-t border-white/10 bg-gradient-to-b from-black to-zinc-950">
        <div className="max-w-4xl mx-auto text-center space-y-12 animate-fade-in-up" style={{ animationDelay: "0.8s" }}>
          ...
          <div className="pt-8">
            <a href="https://form.respondi.app/lDg0Bifo" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 px-8 py-5 bg-[#2bffa0] hover:bg-[#2bffa0]/90 text-black rounded-none font-black tracking-widest transition-all transform hover:scale-105 hover:shadow-[0_0_40px_-10px_rgba(43,255,160,0.5)] group">
      ...
      {/* Floating CTA */}
      <div className={`fixed bottom-10 left-1/2 -translate-x-1/2 z-50 transition-all duration-700 ease-in-out ${isIdle ? "opacity-100 translate-y-0 scale-100 pointer-events-auto" : "opacity-0 translate-y-10 scale-95 pointer-events-none"}`}>
        <a href="https://form.respondi.app/lDg0Bifo" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-8 py-4 bg-[#2bffa0] text-black font-black uppercase tracking-widest text-sm shadow-[0_0_50px_rgba(43,255,160,0.3)] rounded-full hover:scale-105 transition-transform">
          Iniciar Diagnóstico
